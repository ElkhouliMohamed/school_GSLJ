...
export default function Index({ facilities }) {
    ...
    return (
        <AdminLayout title="Facilities & Services">
            <Head title="Manage Facilities" />

            <div className="sm:flex sm:items-center">
                <div className="sm:flex-auto">
                    <p className="mt-2 text-sm text-gray-700">Manage school facilities and services (Transport, Catering, Uniforms, Labs, etc.).</p>
                </div>
                            ...
                            <table className="min-w-full divide-y divide-gray-300">
                                ...
                                <tbody className="divide-y divide-gray-200 bg-white">
                                    {facilities.data.map((facility) => (
                                        <tr key={facility.id}>
                                            <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{facility.name.en || facility.name}</td>
                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                <span className="capitalize">{typeLabels[facility.type] || facility.type}</span>
                                            </td>
                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                {facility.is_active ? (
                                            ...
                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{facility.order}</td>
                                            <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                                <Link href={route('admin.facilities.edit', facility.id)} className="text-violet-600 hover:text-violet-900 mr-4">
