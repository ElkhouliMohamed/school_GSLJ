...
export default function Index({ programs }) {
    ...
    return (
        <AdminLayout title="Educational Programs">
            <Head title="Manage Programs" />

            <div className="sm:flex sm:items-center">
                <div className="sm:flex-auto">
                    <p className="mt-2 text-sm text-gray-700">Manage educational programs and cycles (Preschool, Elementary, Middle, Secondary).</p>
                </div>
                            ...
                            <table className="min-w-full divide-y divide-gray-300">
                                ...
                                <tbody className="divide-y divide-gray-200 bg-white">
                                    {programs.data.map((program) => (
                                        <tr key={program.id}>
                                            <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{program.name.en || program.name}</td>
                                            ...
                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{program.order}</td>
                                            <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                                <Link href={route('admin.programs.edit', program.id)} className="text-violet-600 hover:text-violet-900 mr-4">
